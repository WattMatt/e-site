 ...
 *     row in place (00202 sets rfi.gatekeeper_rule = 'creator'), and the
 ...
 *     equal. Amendments are parsed with `--` comments blanked (00202's header
 ...
 *  rule the locator uses): 00202's header carries the rollback statement — the
...
describe('work-item type registry — A(b) <-> migration <-> TypeScript', () => {
  ...
  it('parses a non-empty set from each source (a parser that matched nothing would make every assertion vacuous)', () => {
    ...
    // 00202 amends rfi.gatekeeper_rule; a parser that matched nothing would let
