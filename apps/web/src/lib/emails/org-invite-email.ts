...
export interface SendInviteLinkEmailParams {
  to: string
  orgName: string
  inviterName: string
  actionLink: string
...
/**
 * Sends a "you've been invited" email with a direct accept-link.
 * Used for new-user invites (Path A) where Supabase generates the link.
 *
 * Never throws: all errors are returned as { ok: false, error }.
 */
export async function sendInviteLinkEmail(
  params: SendInviteLinkEmailParams,
): Promise<{ ok: boolean; error?: string }> {
  try {
    const apiKey = process.env.RESEND_API_KEY
    const from = process.env.RESEND_FROM
    ...
    const { to, orgName, inviterName, actionLink } = params
    ...
    <tr>
      <td>
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:8px;padding:40px 36px;">
          ...
          <tr>
            <td style="font-size:16px;color:#1a1a1a;line-height:1.6;padding-bottom:16px;">
              You've been invited to E-Site by <strong>${escapeHtml(inviterName)}</strong> to join <strong>${escapeHtml(orgName)}</strong>.
            </td>
          </tr>
          <tr>
            <td style="font-size:15px;color:#444444;line-height:1.6;padding-bottom:32px;">
