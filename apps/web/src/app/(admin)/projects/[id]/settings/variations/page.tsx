...
import { listVariationOrdersAction } from '@/actions/variation.actions'
...
export default async function Page({ params }: Props) {
  ...
  // A variation order adjusts the current BOQ import — gate on it existing.
        ...
        <CardBody>
          <div style={{ textAlign: 'center', padding: '32px 16px' }}>
            <h3 style={{ margin: '0 0 6px', fontFamily: 'var(--font-sans)', fontSize: 15, fontWeight: 600, color: 'var(--c-text)' }}>
              No BOQ to vary against
            </h3>
            <p style={{ margin: 0, fontSize: 13, color: 'var(--c-text-mid)', fontFamily: 'var(--font-sans)', maxWidth: 420, marginInline: 'auto' }}>
              Import a BOQ on the Rates tab first. Variation orders adjust quantities on, or add items to, the priced contract Bill of Quantities.
            </p>
          </div>
        </CardBody>
      </Card>
  ...
  const vosRes = await listVariationOrdersAction(id)
  const vos = 'data' in vosRes ? vosRes.data.vos : []
