...
import { UnlockInspectionsButton } from './UnlockInspectionsButton'

export const dynamic = 'force-dynamic'
export const metadata: Metadata = { title: 'Unlock Inspections' }

const BENEFITS = [
  'Build and version your own inspection templates',
  'Access all current and future templates we publish',
  'Capture inspections on web and mobile (offline-ready)',
  'Auto-numbered Certificates of Compliance + sign-off flow',
  'Lifetime access — pay once, no renewals',
]

export default async function UnlockInspectionsPage() {
  ...
  // Already unlocked — send the user straight to the template library.
  const unlocked = await hasFeature(mem.organisation_id, 'inspections', supabase)
  if (unlocked) redirect('/inspections/templates')

  const isAdmin = ['owner', 'admin'].includes(mem.role)
  const price = FEATURE_PRICES.inspections
    ...
    <div className="animate-fadeup" style={{ maxWidth: 720 }}>
      ...
      <Card className="animate-fadeup-1">
        <CardBody>
          ...
          {isAdmin ? (
            <UnlockInspectionsButton label={`Unlock for ${formatZARFromKobo(price.amountKobo)}`} />
          ) : (
            <div
              style={{
                padding: '12px 14px',
