...
- **`public.custom_jwt_claims` (00164) mirrors `user_has_project_access` clause (a) *without* the membership flag — by design, "keeping mobile == web".** Its own query, not a call to the helper, so it does not inherit `00197`. After `00197` that parity breaks in the web-safe direction only: a soft-deactivated member loses every database read, but their next mobile token still lists the project in `project_ids`, so the PowerSync sync rule keeps syncing it (mobile revocation was already eventually-consistent — the claim is minted per token, ~1 h). Left unchanged because it is a caller relying on the old behaviour on purpose and the hook runs on every token mint; the fix is the same one-line predicate and is the owner's call. **Same class, also untouched:** `inspections.user_has_inspection_read`, `inspections.user_can_write_responses` and `inspections.user_can_verify` carry their *own* `project_members` joins without `pm.is_active` (the first and third without `uo.is_active` either), so a deactivated member can still read, write responses to and verify inspections over PostgREST. Today there are 0 deactivated rows, so both are latent.
