...
>
> `resendInviteAction` (added with the invite-expiry fix) re-sends the branded set-password invite — fresh recovery link + 6-digit code — to any **active member of the caller's org**. It is deliberately NOT gated on `last_sign_in_at`: GoTrue sets that field when a recovery link is merely *verified* (server-side `verifyOtp` in `/auth/callback`), so a scanner-prefetched or abandoned invite link makes a stranded invitee look "signed in" — exactly the user who needs the resend (proven in prod 2026-07-07: token consumed, 0 sessions, no password). Resending is idempotent (re-mints the recovery token, never touches an existing password); the UI adds a confirm step when a sign-in is recorded, and the audit event logs `had_prior_sign_in`. Sub-org members are **out of scope** (their membership row is on the sub-org, not the caller's org) — for them, removing and re-adding via the sub-org roster re-sends the invite. Rate-limited per caller like `createUserAction`; surfaced as the "Resend invite" button on `/settings/users` rows.
