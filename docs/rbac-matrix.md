...
### RFIs (`rfi.actions.ts`, `packages/shared/src/services/rfi.service.ts`)

Cells describe the caller's **effective project role** on the RFI's own project (`public.user_effective_project_role`, `00107`), never org membership. Migration `00201` is the database backstop for every one of these rows.

| Action | owner | admin | project_manager | contractor | inspector | supplier | client_viewer |
|---|---|---|---|---|---|---|---|
| `createRfiAction` (raise) | W | W | W | W | W | W | — |
| `respondToRfiAction` (answer, `→ responded`) | W | W | W | W | W | W | — |
| `closeRfiAction` (`→ closed`) | W | W | W | raiser only | raiser only | raiser only | — |
| reopen (`closed → open`; no UI, PostgREST only) | W | W | W | W | raiser only | raiser only | — |
| route / re-date (`assigned_to`, `due_date`, `priority`, `category`, `subject`, `description`; no UI yet) | W | W | W | W | — | — | — |
| move project / organisation, rewrite `raised_by` or `created_at` | — | — | — | — | — | — | — |

> **The gap `00201` closes.** `00027 §3` gave `projects.rfis` its only UPDATE policy — `USING (organisation_id = ANY(get_user_org_ids()))`, no `WITH CHECK`, no role predicate, no column restriction — so **every active org member held full UPDATE on every RFI in the organisation, including RFIs on projects they hold no role on.** `closeRfiAction` checked authentication only, and both the web and mobile Close buttons were rendered for any role that could see the record. Reproduced on production 2026-09-14 in a rolled-back transaction, as the `rbac-test` contractor, in ONE statement: RFI-4 (raised by the org owner) closed by the contractor, `closed_by` forged to the raiser, `closed_at` backdated 400 days, the row moved to a project the contractor holds no role on, `due_date` pushed to 2036 and an assignee invented. Nothing refused any part of it.
>
> **Who may close: the raiser, or a governing role** (`owner`/`admin`/`project_manager` — `ORG_WRITE_ROLES`). This is §03 §1.8's "only the gatekeeper closes" written on the source table: after `00198` the RFI work item's `gatekeeper_rule` is `'creator'` and a mirrored RFI item's `created_by` is `raised_by`, and the governing arm is `00196` §12 (b)+(d)'s "a governing actor takes the seat and closes in one statement", collapsed — `projects.rfis` has no gatekeeper column to take. **This refuses nothing production has ever done:** of 6 closed RFIs, 2 were closed by their raiser and 4 by the org owner; **zero** by someone who was neither (measured 2026-09-14, read-only). 12 of 15 RFIs were raised by contractors and none of those closed themselves.
>
> **Answering is deliberately wider than routing.** `projects.rfi_responses` admits any non-`client_viewer` org member (`00009`/`00161`), so the `open → responded` flip that accompanies an answer must not be what fails for an `inspector` or `supplier`. Routing and content are the `rfi` **write set** — the four roles `projects.work_item_types.write_roles` gives the type (`00196:239`), the same four as `MARKUP_WRITE_ROLES` on this surface — per §03 §1.8/§1.9 ("a write-role holder may assign/re-date"). **Reopening** is the write set **or** the raiser, which is `00196` §12 (c2) exactly.
>
> **Three things the guard owns that RLS cannot express**, because RLS cannot compare `OLD` and `NEW` (the `00172:249-267` / `00196` §12 pattern): the **status machine** (`draft → open|closed`, `open → responded|closed`, `responded → open|closed`, `closed → open`, and nothing else — so a closed RFI cannot be silently re-answered); the **stamps** (`closed_at`/`closed_by` are the guard's — a `closed_by` naming anyone but the caller is refused with a sentence, a backdated `closed_at` is overwritten, and a save that does not change the status has both restored from `OLD`); and the **fixed columns** (`project_id`, `organisation_id`, `raised_by`, `created_at` — a row that can move carries its own authority with it). The guard compares `auth.uid()`, never `current_user`.
>
> **A refusal by the RESTRICTIVE policy is SILENT** — a policy that matches no row raises nothing, and PostgREST returns `200` with an empty array. `closeRfiAction`, `rfiService.close`, `respondToRfiAction` and `rfiService.respond` therefore **assert rows affected** (the `E8` lesson) rather than checking only `error`. A refused *answer* never discards the response: the row is kept and the caller is told the status did not move.
>
> **Exempt from authorisation, never from the stamps: `auth.uid() IS NULL` and `pg_trigger_depth() > 1`.** The first is the service client and migrations (`00198`'s backfill supplies a historical `closed_at` and its own `closed_by`, and both survive). The second is item 3's assignment/due-date write-back, which updates `projects.rfis` from an `AFTER` trigger on `projects.work_items` **in the source writer's session** — where `auth.uid()` is frequently a contractor with no right to re-date by hand. It is `00196:1590-1605`'s documented bypass, and the assertion file exercises it with a stand-in trigger at the same depth so the arm is not left untested until `00198` lands.
>
> **What narrowed.** (1) An org member with **no effective role** on the RFI's project loses UPDATE on it — 43 of 108 (member × project) pairs across the four RFI-bearing projects, 15 distinct people, every one an org `contractor` or `client_viewer` (measured 2026-09-14). They keep their read (`00034`'s SELECT policy is org-wide for a non-viewer) and can still write a response row. (2) A **project-scoped `client_viewer`** — `projects.project_members.role = 'client_viewer'` while the org role is something else — loses UPDATE; `00161`'s block reads `public.user_is_client_viewer`, which looks only at `public.user_organisations`, so it never fired for that shape. (3) `inspector` and `supplier` lose routing and re-dating, which they were never in the `/rfis` write set for.
...
- **`projects.rfis` INSERT is still org-scoped and `raised_by` is caller-supplied.** `00201` closed the UPDATE side; `"Org members can create rfis"` (`00027 §3`) still checks only `organisation_id = ANY(get_user_org_ids())`, so any non-`client_viewer` org member can raise an RFI **on a project they hold no role on**, naming anyone as `raised_by` (nothing binds it to `auth.uid()`). Not closed with the UPDATE gate because it changes what `/rfis/new` can offer — that page lists every project the caller can see, and a role predicate there is a UI decision, not a backstop. **The interaction is contained**: an RFI inserted with `raised_by = auth.uid()` on a foreign project still cannot be UPDATEd by its inserter, because `rfis_update_gate`'s USING requires an effective role on that project. The clean fix is a RESTRICTIVE INSERT policy pinning `raised_by = auth.uid()` and calling `projects.user_can_update_rfi(project_id)`, shipped with the `/rfis/new` project list narrowed to match.
- **`projects.rfi_responses` writes are org-scoped, not project-scoped.** Its `00009` INSERT policy and `00027 §7` UPDATE policy both qualify on the parent RFI's organisation; `00161` excludes only an **org-level** `client_viewer`. So an org member with no role on the project can still write and edit answers there, and a **project-scoped** `client_viewer` can too — the same shape `00201` closes on the parent table. Deliberately out of scope: an answer is not a status change, and narrowing the answer surface is a product decision about who may participate in an RFI thread.
