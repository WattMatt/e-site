...
|---|---|---|---|---|---|---|
| `rfi` | Q1 | `projects.rfis` | +7 wd | office | project `triage_owner_id` | the raiser (`rfis.raised_by`), else the project PM — registry `gatekeeper_rule = 'creator'` ¹ |
...
> ¹ **`rfi`'s gatekeeper is the raiser, not the PM** (owner decision; measured 2026-09-10). `triage_owner_id` and the §1.5 PM resolver both return the same person on 13 of 14 projects, so a PM gatekeeper makes assignee and gatekeeper identical on all 15 live RFIs and §03 §1.8's "only the gatekeeper may close" vacuous for the type that carries metric 4. An RFI is the mirror image of a snag: the asker confirms the answer is usable, whereas a defect is never signed off by the person who reported it. 12 of the 15 live RFIs were raised by contractors, and this is the only Q1 mechanism that puts a work item into a contractor's ball-in-court.
>
> ⚠ **The consequence, stated plainly: a contractor who raises an RFI IS its gatekeeper and CAN close it.** Any sentence elsewhere claiming only owner/admin/PM closes an RFI-mirrored item is wrong. An **ineligible** raiser falls back to the project PM. A contractor who merely holds the ball on someone *else's* RFI still cannot close it — clause (d) refuses with `P0001 Only the person who signs RFI-1 off can close it. Take it over first, or ask them to close it.` Changing the **gatekeeper seat** on an existing item still requires governance (owner/admin/PM) — that is item 2's clause and is unchanged. Measured on the four arms of `work_items_select` for a contractor on their own mirrored item: `project_access = true`, `assignee = false`, `gatekeeper = true`, `watcher = true`.

**Six of the seven Q1 sources get a projection trigger, plus a seventh, report-level entry point on `projects.qc_reports`** (the `qc_defect` scope predicate spans two tables, so an entry never crosses into scope on its own). **`structure.node_orders` does not** — `order_followup` is created only by the explicit chase control on an order line, and the Q1 migration ledger in A(f) is written that way.
...
|---|---|---|
| **Q1** | `work_items`, `work_item_types`, `work_item_events`, `work_item_watchers`, `activity`, `public_holidays`, `calendar_years`, `working_days_between()`, `project_had_activity()`, `add_working_days()`, `push_past_builders_shutdown()`, `org_owner()`, `resolve_project_pm()`, `resolve_triage_owner()`, `resolve_work_item_assignee()`, `user_can_read_work_item()`, `user_can_write_work_item()`, `backup_00202_source_assignees`, `work_item_person_eligible()`, `resolve_mirror_assignee()`, `resolve_work_item_gatekeeper()`, `map_source_status()`, `work_item_status_for_mirror()`, `work_item_mirror_due_date()`, `diary_delay_text()`, `project_rfi()`, `project_snag()`, `project_inspection()`, `project_qc_entry()`, `project_diary_action()`, `project_form_action()`, `mirror_rfi_work_item()`, `mirror_snag_work_item()`, `mirror_inspection_work_item()`, `mirror_qc_defect_work_item()`, `mirror_qc_report_defects()`, `mirror_diary_action_work_item()`, `mirror_form_action_work_item()`, `work_item_assignment_writeback()`, `void_work_item_on_source_delete()` | `projects` |
...
`working_days_between()` counts working days between two dates and is created by the Q1 ordinal-1 metrics migration; `add_working_days()` is its inverse and is created by ordinal 7, because the `BEFORE INSERT` due-date trigger has to add days rather than count them. Trigger functions (`work_items_set_due_date`, `work_items_ensure_ref`, `work_items_assert_membership`, `work_items_transition_guard` (replaced by `00202`: depth-scoped exemption), `append_work_item_event`, `validate_work_item_defaults`, `ensure_project_settings_row`) are declared in each migration's `-- @verify:` block and are covered by test 8 through it.

`00202`'s twenty-two new functions divide into **eighteen stateful** ones — `SECURITY DEFINER … SET row_security TO 'off'`, `SET search_path`, revoked from PUBLIC **and** `anon`, granted to nobody (a trigger function fires without `EXECUTE`) — and **four pure mappers** (`map_source_status`, `work_item_status_for_mirror`, `work_item_mirror_due_date`, `diary_delay_text`) that are `LANGUAGE sql` invoker, `IMMUTABLE`/`STABLE`, read no table and carry no `SET search_path`. The count is derived from the migration file itself, not from this table.
...
| **1** | Metrics, presence and calendar foundation (§15's Q1 metrics migration): `product_events`, `platform_metrics_weekly`, `metric_cohorts`, the `metric_accounts` view, `user_presence`, `user_sessions`, `touch_presence()`, `user_is_org_admin()`, `projects.public_holidays` seeded from `listHolidaysNamed(year)`, `projects.calendar_years`, `working_days_between()`, and the weekly rollup job (`0 4 * * 1`) | The baseline every later target is measured against, **and** A(h)'s calendar, which must precede the spine because `due_date` is `NOT NULL` and its trigger raises `no_data_found` on an unseeded year |
