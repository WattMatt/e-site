...
- **Tenant loading:** `loadKw = participation === 'shared' ? (manual_kw_override ?? area_m2 × rate[category]) : 0` (`own`/`none` → 0), `rate` from settings (defaults: standard/national 0.03, fast_food/restaurant 0.045).
...
- **Capital recovery (PMT):** `annual = capex × (r(1+r)^n)/((1+r)^n − 1)`, `monthlyCapitalRepayment = annual/12`, `r = rate%/100`, `n = years`.
- **Operational tariff (R/kWh):** `netKva = largestGenKva × runningLoad%/100`; `netKwh = netKva × powerFactor`; `dieselR/kWh = (fuelConsumption(largestGenSize, runningLoad) × dieselPrice × runningHours) / netKwh`; `maintR/kWh = (maintMonthly + max(0, serviceCostPer250h − maintMonthly)) / netKwh`; `finalTariff = (dieselR/kWh + maintR/kWh) × (1 + contingency%/100)`.
