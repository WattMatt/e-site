...
> **Reconciliation 2026-06-09 — engine BUILT + golden-master-verified** against nexus's *billed PDF* path (`packages/shared/src/services/generator-cost-recovery/`, `__fixtures__/nexus-golden/`, 27/27). This caught & fixed real divergences: diesel no longer ×runningHours, the fuel interpolation, the rate mapping, and the maintenance formula. **Capital recovery (PMT) was then re-based to WM's real Rev-6 report (ANNUAL ÷ 12 → R65 710.68) — the authoritative source — reversing nexus's monthly-compounding code and fixing a 2.72% under-charge (`2017e7e`).** **WM decisions (2026-06-09):** (1) nexus `main` is the canonical **base to improve & expand on** (not a slavish clone); (2) maintenance **clamped at zero** ✅ (never reduces the tariff); (3) **honour 4 distinct category rates** ✅ done `5871263` (deliberate divergence from nexus's 2-tier); (4) opt-out (`none`) tenants **pay nothing** (excluded from capex + apportionment) and are **greyed out** in the UI.
...
- **Tenant loading:** `loadKw = participation === 'shared' ? (manual_kw_override ?? area_m2 × rate[category]) : 0` (`own`/`none` → 0). **Rate mapping (WM: honour 4 distinct rates) — each category → its own rate** (`standard`→standardKwPerSqm · `fast_food`→fastFoodKwPerSqm · `restaurant`→restaurantKwPerSqm · `national`→nationalKwPerSqm · `other`→standard). A deliberate divergence from nexus's 2-tier billing.
...
- **Capital recovery (PMT — ANNUAL compounding ÷ 12, per WM's Rev-6 report):** `annual = capex × r(1+r)^n / ((1+r)^n − 1)` (`r = rate%/100`, `n = years`); `monthlyCapitalRepayment = annual / 12`. Anchored to WM's real report (R4 455 360 @12%/10y → **R65 710.68/mo**; R1 000 000 → R14 748.68). Deliberately diverges from nexus's *code* (monthly compounding), which under-charged 2.72%.
- **Operational tariff (R/kWh — reconciled to nexus billed path):** `largestGen` = max by `parseInt(size)` kVA across all zones; `netKva = largestGenKva × runningLoad%/100`; `netKwh = netKva × powerFactor`; **`dieselR/kWh = (fuelConsumption × dieselPrice) / netKwh`** (cost *per hour* ÷ netKwh — running hours are **not** a factor); **`maintR/kWh = additional / (netKwh × runningHours)`** where `additional = max(0, (runningHours/250)×maintenanceCostAnnual − maintenanceCostAnnual/12)` (**clamped at zero per WM** — never reduces the tariff); `finalTariff = (dieselR/kWh + maintR/kWh) × (1 + contingency%/100)`.
...
- **Document structure — full WM standby-power report (caters for the Rev-6 example, `PMM — Standby System`):**
  - **Cover** — branded (4 logos + accent + parties strip + footer) + **revision (Rev N)** + **issuer block** (firm name, address, tel, contact person).
  - **Glossary · Table of Contents.**
  - **Narrative (per-project, editable):** Introduction · **Plant Sizing** (basis: category kW/m² + food-outlet / larger-supply exceptions) · Outline of System (selective metering, switching philosophy, cost-of-occupation goals) · Switching System (essential / non-essential split) · Operational Functionality (main-board control, per-shop key switches, per-lease on/off) · Recommendations.
  - **Figures** — embedded engineering diagrams (typical DB line diagram, system schematic, …); image support.
  - **Plant Sizing summary** — plants → main boards (kVA) + spare capacity + power-factor allowance → plant total capacity (sizes the genset).
  - **Appendix A — Capital cost** (plant cost · supply cabling · main-board + shop-board amendments · control wiring → total capex).
  - **Appendix B — Capital recovery + operational tariff** — the **full N-year amortisation schedule** (Year · Beginning · PMT · Interest · Principal · Ending) + diesel/operational detail (net energy kVA→kWh · fuel · maintenance · contingency → R/kWh).
  - **Appendix C — Tenant load allocation** — shop · tenant · area · expected load · % of total · monthly rental · R/m², **grouped by Centre/zone**, with **VAT (excl/incl)**; opt-outs shown greyed as "Not on generator — R0"; reconciliation Σ = monthly repayment.
- **New data the full report needs (P2 schema + P3 report — beyond the P1 cost engine):** a **main-board sizing** layer (`gcr.main_boards`: kVA load, spare capacity, power-factor allowance per plant); **per-project narrative** (editable rich-text per section); **figures** (image storage + register, embedded by react-pdf); a **VAT %** setting (excl/incl presentation); revision surfaced from `projects.reports.version`; issuer firm address/tel/contact from the org profile. The P1 engine already covers the cost maths these tables present.
...
8. **WM decisions 2026-06-09** — nexus is the **base to improve & expand on** (not a slavish clone); **honour 4 distinct category rates** (done, `5871263`); opt-outs **pay nothing + greyed out**; maintenance **clamp-at-zero** pending Arno's final nod.
