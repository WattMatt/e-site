...
| `docs/superpowers/specs/2026-09-09-v2-platform-roadmap/16-appendix-registries.md` | **Modify.** A(f)'s Q1 row **already lists** `resolve_project_pm()`, `resolve_work_item_assignee()`, `user_can_*` and item 2's trigger functions (`:331`, `:350`); it gains **only the new names** — the twenty-two functions, `resolve_mirror_assignee()` among them, and `backup_00199_source_assignees`; A(f)'s ordinal-9 sentence gains `qc_reports` and loses `auth_events.session_id`; A(b)'s `rfi` gatekeeper cell changes to the raiser (owner decision, with the registry `UPDATE`). |
...
**The number is claimed here, not at Task 20.** Ledger `max(version) = '00196'` (item 2, applied 2026-09-12); open PR #185 (the spun-off `user_has_project_access` `is_active` fix) already holds **`00197`**. This migration is **`00199`**. Task 20 Step 1 re-checks the ledger, `origin/main` and every open PR at merge; if `00199` has been taken by then, take the next free number and rename in one pass (Task 20 Step 4). ⚠ Placeholders are not an option: `parseVerifyBlock` requires bare identifiers (`/^[a-z0-9_]+$/i`, `verify-header.ts:90,103`) and the hygiene contract test scans every migration `>= '00185'` — a `<NNNNN>` or a `99999_` file makes `pnpm --filter web test` red from this task's Step 5 onward.

**Files:**
- Create: `apps/edge-functions/supabase/migrations/00199_work_item_source_mirrors_and_backfill.sql`
...
- [ ] **Step 3: Create the migration file with its header and section A.** Create `apps/edge-functions/supabase/migrations/00199_work_item_source_mirrors_and_backfill.sql`:
  ```sql
  -- =============================================================================
...
- [ ] **Step 5: Commit.**
  ```bash
  git add apps/edge-functions/supabase/migrations/00199_work_item_source_mirrors_and_backfill.sql \
          scripts/db/probes/01-preflight.sql
  git commit -m "feat(work-items): 00199 skeleton, @verify header and pre-flight assertions for source mirrors
...
- [ ] **Step 7: Extend item 2's harness and run its ten assertion files with `00199` stacked.** In `scripts/db/try-work-item-spine.sh`, after the `PRELUDE` block and before `SQL=$(printf …)`, add:
  ```bash
  ...
  for f in scripts/db/assertions/work-item-*.sql; do
    WITH_EXTRA=apps/edge-functions/supabase/migrations/00199_work_item_source_mirrors_and_backfill.sql \
...
the mirror does. Item 2's ten assertion files pass with 00199 stacked."
  ```
...
- [ ] **Step 1: Write the probe first.** Create `scripts/db/probes/12-grants.sql`:
  ...
  SELECT 'snapshot_table_not_anon_readable',
         NOT has_table_privilege('anon', 'projects.backup_00199_source_assignees', 'SELECT'),
  ...
  ```
  The snapshot table is named `backup_00199_source_assignees` from the start (A(f)'s R52 convention `backup_<version>_<object>`; renamed with the migration on 2026-09-15). If Task 20 Step 1 forces a different number, the rename touches this probe in the same commit.
...
- [ ] **Step 6b: Retarget the four item-2 fixtures that will collide with the backfill (#14), and run item 2's suite with `00199` stacked.** `scripts/db/assertions/work-item-ddl.sql:119,123`, `work-item-events.sql:111` and `work-item-transition.sql:155,183` insert `origin='mirror'` rows against **live** `projects.rfis` ids. After section H, every non-demo RFI already carries a mirror and `work_items_src_rfi_uidx` allows one — the four files go red with `23505` on the day item 3 lands. First reproduce that:
  ...
  ```sql
  -- 00199's live trigger mirrors this RFI on insert; the fixture owns the mirror it
  -- asserts on, so it removes the trigger-made row first (0 rows before 00199
  ...
  ```
  Expected: `✓` for all ten. Then run them **unstacked** (`scripts/db/try-work-item-spine.sh "$f"` with no `WITH_EXTRA`) and confirm all ten still pass — the retargeted fixtures must be correct both before and after `00199` applies, because item 2's suite is also the post-apply regression check (Task 20 Step 8).
...
assertion files pass with 00199 stacked and unstacked."
  ```
...
- [ ] **Step 3: Run the full rehearsal, and the four stateless probes beside it.**
  ...
  # Item 2's own regression suite, with 00199 stacked (Task 15 Step 6b retargeted
...
- [ ] **Step 8: Confirm no mail could have been sent by the migration itself.**
  ...
  grep -c "send-email\|net.http_post\|pg_net\|fetch(" \
    apps/edge-functions/supabase/migrations/00199_work_item_source_mirrors_and_backfill.sql
  ```
...
- [ ] **Step 4: Extend Appendix A(f)'s Q1 row — new names only.** In `16-appendix-registries.md`, the A(f) table's **Q1 / `projects`** cell already lists `resolve_project_pm()`, `resolve_work_item_assignee()`, `user_can_*` and item 2's trigger functions (`:331`, `:350`). **Do not repeat them.** Append after the last existing entry:
  ```
  , backup_00199_source_assignees, work_item_person_eligible(), resolve_mirror_assignee(),
  ...
  ```
  and beside item 2's `work_items_transition_guard()` entry add "(replaced by 00199: depth-scoped exemption)". If the owner takes option (a) in Task 20 Step 5, `project_settings.suppress_all_outbound` is booked under ordinal 6's column list with "(added by 00199)".
...
**Files:**
- Rename: the migration file — **only if** Step 1 finds `00199` taken
...
- [ ] **Step 1: Re-read `max(version)` from production AND `origin/main`, at merge time, not now.**
  ...
  ```
  As of 2026-09-13 the ledger reads **`00196`** (item 2, applied 2026-09-12), `origin/main`'s tail is `00196`, and open PR #185 holds **`00197`** — which is why this file has been `00199` since Task 2. **Re-read all three.** If #185 has merged and applied, `max(version)` reads `00197` and `00199` is still right; if something else has taken `00199`, take the next free number and do the rename in Step 4. ⚠ Merge-order: if #185 has **not** applied by the time this merges, `00199` landing first makes `db push` refuse #185's `00197` ("local migration files to be inserted before the last migration on remote" — the PR #163/#165 lesson). Tell the owner which order the two must apply in; do not silently take `00197`.
...
- [ ] **Step 4: Re-derive the literals; rename only if the number moved.** The file, the snapshot table (`backup_00199_source_assignees`), the `@verify` block, probes 12 and 13 and the header have carried `00199` since Task 2 (placeholders do not parse — Task 2). In one edit pass:
  ...
  3. **only if Step 1 found `00199` taken:** `git mv` the file to the new number and change every `00199` in the header, the `table:`/`grant_absent:` lines, sections H's four snapshot-table references, the two restore statements, probes 12 and 13, CONFORMANCE and A(f) — one `grep -rn 00199` over the branch is the checklist.
...
- [ ] **Step 7: A green workflow is not evidence. Read the objects back.**
  ```bash
  node --experimental-strip-types scripts/verify-migration-applied.ts --since 00197   # > 00197 ⇒ 00199 only
  ```
  (`--since` keeps files whose 5-digit prefix is **greater** than the value — `scripts/verify-migration-applied.ts:94`; `--file 00199_work_item_source_mirrors_and_backfill.sql` is the other form.) It parses the `-- @verify:` block and evaluates every directive — `table`, `function`, `trigger`, `grant_absent`, and the two `sql:` predicates (the guard's exemption text; the `rfi` registry rule) — one read-only request per predicate, exiting non-zero on any absence, any surviving `anon` privilege, or a false predicate. The deploy workflow already runs this after `db push`; run it again by hand and paste the directive count into the PR body.
...
- [ ] **Step 8: Independently read the data back, which is the check `db push` cannot fake.**
  ...
  SELECT 'version_in_ledger' AS probe,
         EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version = '00199') AS ok,
  ...
  for f in scripts/db/assertions/work-item-*.sql; do           # item 2's suite, UNSTACKED — 00199 is live now
    scripts/db/try-work-item-spine.sh "$f"
  done
  ```
...
- [ ] **Step 11: Write the PR body.** It must carry, at minimum:
  - the claimed number (`00199`) and the `max(version)` + `origin/main` + open-PR readings that justified it, and the apply order agreed with #185;
  ...
  - the full rehearsal output from Task 17 Step 3 with its 109-name `assertions seen:` line, the four stateless probes' 49, item 2's ten `✓` lines with `00199` stacked, plus the deliberate single-arm break from Step 4;
