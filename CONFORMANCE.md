...
Last updated: 2026-09-15 (Q1 item 3 — source mirrors and the backfill, branch `feat/q1-source-mirrors`: C12).
...
| C12 | Work-item mirrors write source columns through SECURITY DEFINER triggers; the transition guard's exemption is depth-scoped | MUST | ✓ | `00202_work_item_source_mirrors_and_backfill.sql` sections C′–G. **Twenty-two new functions** (counted from the file: 23 `CREATE OR REPLACE FUNCTION` statements minus C′'s replacement of item 2's `work_items_transition_guard()`). The **eighteen stateful** ones are `SECURITY DEFINER … SET row_security TO 'off'` **with** `SET search_path`; the **four pure mappers** (`map_source_status`, `work_item_status_for_mirror`, `work_item_mirror_due_date`, `diary_delay_text`) are `LANGUAGE sql` invoker, `IMMUTABLE`/`STABLE`, read no table and carry **no** `SET search_path` — measured, correcting the plan text's "all with `SET search_path`". None uses `current_user` for authorisation (asserted in-migration and by probe, comment-stripped). All revoked from PUBLIC **and** `anon` and granted to nobody — trigger functions fire without EXECUTE, verified with `has_function_privilege`, never by reading `proacl`. ⚠ In schema `projects` there is no function `pg_default_acl`, so `anon`'s EXECUTE **is** PUBLIC's built-in grant: dropping the `FROM anon` line alone is silent, and the proving mutation is dropping `FROM PUBLIC`. **The layering, stated honestly (grants → definer chain → RLS):** on the shipped path a contractor's `INSERT INTO projects.rfis` never reaches `work_items_insert_gate` — `mirror_rfi_work_item()` is SECURITY DEFINER, so the whole projection runs as postgres, which simultaneously satisfies section G's EXECUTE revokes and makes `SET row_security TO 'off'` effective; strip the definer and the EXECUTE revoke answers first (`42501` on `project_rfi`), and only after **also** granting EXECUTE does item 2's RESTRICTIVE spine gate become the thing that refuses. ⚠ That three-layer mutant is a **counterfactual** (it grants what section G revokes), so it is not evidence that RLS protects the shipped path; the unmutated proof that the spine gate bites a real client session is probe 16 arm 3, `contractor_cannot_write_that_row_by_hand`. The load-bearing definer is the **wrapper** `mirror_<src>_work_item()`; `project_<src>()`'s own `SECURITY DEFINER` is defence-in-depth, its only callers being that wrapper and the backfill, both already postgres. Item 2's `work_items_transition_guard()` is `CREATE OR REPLACE`d with `IF v_actor IS NULL OR pg_trigger_depth() > 1` (never `> 0`) and `source_status` in clause (a); proved under impersonation (probe 05b) and pinned by a `sql:` directive on the CODE form plus a contract test. The pre-migration snapshot table `projects.backup_00202_source_assignees` is `REVOKE SELECT … FROM anon` + RLS-enabled with no policy. Authority is checked **once**, on the `projects.work_items` UPDATE, by item 2's `user_can_write_work_item` + RESTRICTIVE policy; the mirror's own resolver (`resolve_mirror_assignee`) has no caller guard and excludes `client_viewer`. |
