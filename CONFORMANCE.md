...
Last updated: 2026-09-15 (RFI write authority — migration `00201`: E10, and C11's evidence).
...
| A11 | Guard unit tests | MUST | ✓ (this pass) | `apps/web/src/middleware.test.ts` (session/email/MFA/org gates, plus filesystem-enumerated public-page and self-auth-route contracts) + `apps/web/src/lib/safe-next.test.ts` + `apps/web/src/lib/auth/require-effective-role.test.ts` + `(auth)/auth/callback/route.test.ts` |
...
| E8 | Compliance surfaces reachable by the people they exist for: the marketing-opt-out and data-subject-request pages must clear the session, signed-in-bounce, unconfirmed-email and MFA gates, and must not report success on a write that changed nothing | MUST | ✓ (this pass) | `middleware.ts` `LEGAL_PREFIXES` / `PUBLIC_CONTENT_PREFIXES` (rules 1, 2, 3, 4b) + `PUBLIC_API_PATHS` for the RFC 8058 `POST /api/unsubscribe`; `safe-next.ts` allows the same paths so a stale `?next=` survives login. `actions/unsubscribe.actions.ts` writes the opt-out on the service client **and asserts rows affected** (the anon client matched zero rows under the `id = auth.uid()` UPDATE policy and PostgREST raised no error, so the page said "You're unsubscribed" 246 sends running with 0 of 36 profiles opted out); opt-back-in is authenticated to `auth.uid()`, since the userId is printed in every marketing email. `middleware.test.ts` enumerates `app/(legal)` + `app/(public)` and the self-authenticating `app/api` handlers **from disk**; `lib/email/list-unsubscribe.contract.test.ts` pins that the header's URL resolves to a handler that exists, accepts POST, and is not redirected |
| E9 | Work-item write authorisation: `projects.user_can_write_work_item(project_id, item_type)` resolving `write_roles` from the registry, a RESTRICTIVE INSERT/UPDATE gate (task-only, `origin='manual'`, born `triage`/`open`, no source FK, no closed/void stamp), a SECURITY INVOKER `BEFORE UPDATE` transition guard enforcing "only the gatekeeper closes" (gatekeeper governance = owner/admin/PM; reopen = write role or the closing gatekeeper; void needs a reason), and an append-only `work_item_events` with no write policy and no client grant | MUST | ✓ (this pass) | `apps/edge-functions/supabase/migrations/00196_work_item_spine.sql` §8 (helpers) – §9 (RLS) – §10 (grants) – §11 (append trigger) – §12 (transition guard) – §13 (settings validator); app-layer gate `apps/web/src/actions/work-items.actions.ts` (five verbs — `createWorkItemTaskAction`, `reassignWorkItemAction`, `advanceWorkItemStatusAction`, `setWorkItemDueDateAction`, `voidWorkItemAction` — each `requireEffectiveRole` on the **project**, payloads built field by field, DB sentences returned verbatim, every UPDATE conditioned on the pre-read status and rows-affected asserted; `work-items.actions.test.ts`, 38 tests, 16 mutations); matrix rows in `docs/rbac-matrix.md` (Work items); role-scoped probes in `scripts/db/assertions/work-item-rls.sql` + `work-item-transition.sql` (rolled-back, on production) and post-apply in `scripts/db/smoke-test-work-item-spine.sh` |

| E10 | Source-module write authority: where a module's own table is the system of record for a work item, the table itself must carry the role gate, the column gate and the status machine — a PERMISSIVE `USING (organisation_id = ANY(get_user_org_ids()))` is membership, not authorisation, and an app-layer check on one action is not a gate on a directly-invocable PostgREST surface | MUST | ✓ (this pass, RFIs) | `apps/edge-functions/supabase/migrations/00201_rfi_write_authority.sql` — `projects.user_can_update_rfi(project_id)` (SECURITY DEFINER, `row_security=off`, `auth.uid()`, COALESCEd, `REVOKE … FROM PUBLIC` **and** `FROM anon`, verified with `has_function_privilege`), a RESTRICTIVE UPDATE policy carrying **both** halves, and a SECURITY INVOKER `BEFORE UPDATE` guard enforcing what RLS cannot express (status machine; `closed_at`/`closed_by` are the guard's and a forged closer is refused by sentence; `project_id`/`organisation_id`/`raised_by`/`created_at` fixed; close = raiser or `ORG_WRITE_ROLES`; reopen = the type's write set or the raiser), with the `auth.uid() IS NULL` and `pg_trigger_depth() > 1` exemptions the service path and item 3's write-back need. App-layer gate `apps/web/src/actions/rfi.actions.ts` (`closeRfiAction` resolves raiser-or-governing before the write; both `closeRfiAction` and `respondToRfiAction` **assert rows affected**, because a RESTRICTIVE policy that matches no row raises nothing — the E8 lesson) and `packages/shared/src/services/rfi.service.ts` (the same for mobile, throwing `RFI_CLOSE_REFUSED`); the Close button is rendered only for callers who may close, on web and on mobile. Matrix rows in `docs/rbac-matrix.md` (RFIs); role-scoped probes in `scripts/db/assertions/rfi-update-gate.sql`, run by `scripts/db/try-rfi-update-gate.sh` against production in a rolled-back transaction (`BARE=1` is the red run), 14 mutations of the migration each killed by a named assertion. Unit coverage: `apps/web/src/actions/rfi.actions.test.ts`, `packages/shared/src/services/rfi.service.test.ts` |
