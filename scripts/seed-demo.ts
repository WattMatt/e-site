 ...
 * Usage:
 *   pnpm demo:seed
 *   pnpm demo:seed --env=production   # reads from .env.production.local
 *
...
async function upsertAuthUser(opts: {
  ...
  const { data: existing } = await supabase.auth.admin.getUserById(opts.id)
  if (existing.user) {
    log(`  ↩ Auth user exists: ${opts.email}`)
  ...
  const { data, error } = await supabase.auth.admin.createUser({
    ...
    // Override the generated ID by using the identities approach —
    // Supabase admin createUser doesn't accept custom IDs directly,
    // so we'll update afterwards if the ID differs
  })
  if (error) throw new Error(`createUser ${opts.email}: ${error.message}`)
  log(`  ✓ Created auth user: ${opts.email}`)
  return data.user
}
...
async function seed() {
  ...
  for (const u of users) {
    // Check if user exists by email using admin list
  ...
  const projectMembers = [
    { project_id: IDs.pSandton,   user_id: ownerUser.id,   role: 'project_manager', is_active: true },
    { project_id: IDs.pSandton,   user_id: pmUser?.id,     role: 'project_manager', is_active: true },
    { project_id: IDs.pSandton,   user_id: fieldUser?.id,  role: 'contractor',      is_active: true },
    { project_id: IDs.pMidrand,   user_id: ownerUser.id,   role: 'project_manager', is_active: true },
    { project_id: IDs.pMidrand,   user_id: pmUser?.id,     role: 'project_manager', is_active: true },
    { project_id: IDs.pCenturion, user_id: ownerUser.id,   role: 'project_manager', is_active: true },
    { project_id: IDs.pCenturion, user_id: fieldUser?.id,  role: 'contractor',      is_active: true },
    { project_id: IDs.pCenturion, user_id: clientUser?.id, role: 'client_viewer',   is_active: true },
  ].filter((m) => m.user_id)
  ...
  const sites = [
    {
      ...
      address: '5th Street & Rivonia Road, Sandton',
      city:    'Sandton', province: 'Gauteng',
      erf_number: 'ERF 2847', site_type: 'commercial', status: 'active',
    },
    {
      id: IDs.csMidrand, organisation_id: IDs.org, created_by: ownerUser.id,
      name:    'Midrand Business Estate — Units 1–8',
      address: '14 Waterfall Drive, Midrand',
      city:    'Midrand', province: 'Gauteng',
      erf_number: 'ERF 1092', site_type: 'commercial', status: 'active',
    },
    {
      id: IDs.csCenturion, organisation_id: IDs.org, created_by: ownerUser.id,
      name:    'Centurion Industrial Park — Unit 12',
      address: '42 Akkerboom Road, Hennopspark, Centurion',
      city:    'Centurion', province: 'Gauteng',
      erf_number: 'ERF 556', site_type: 'industrial', status: 'active',
    },
  ]
  ...
  // Subsections per site (realistic SANS 10142-1 COC categories)
  ...
  const orders = [
    {
      ...
      delivery_address: '5th Street & Rivonia Road, Sandton',
    ...
    },
    {
      ...
      delivery_address: '14 Waterfall Drive, Midrand',
      total_amount: 18750.00,
    },
  ]

  const orderLines = [
    { id: 'cccccccc-0000-0000-0002-000000000001', order_id: orders[0].id, catalogue_item_id: catItems[0].id, quantity: 4, unit_price: 185.00, line_total: 740.00 },
    { id: 'cccccccc-0000-0000-0002-000000000002', order_id: orders[0].id, catalogue_item_id: catItems[2].id, quantity: 1, unit_price: 320.00, line_total: 320.00 },
    { id: 'cccccccc-0000-0000-0002-000000000003', order_id: orders[1].id, catalogue_item_id: catItems[3].id, quantity: 150, unit_price: 95.00, line_total: 14250.00 },
    { id: 'cccccccc-0000-0000-0002-000000000004', order_id: orders[1].id, catalogue_item_id: catItems[4].id, quantity: 150, unit_price: 28.50, line_total: 4275.00 },
  ]
  ...
  for (const line of orderLines) {
    const { error } = await (supabase as any).schema('marketplace').from('order_lines').upsert(
      line, { onConflict: 'id', ignoreDuplicates: true }
    )
    if (error) log(`⚠ order line: ${error.message}`)
  }
  log(`✓ ${orderLines.length} order lines`)
  ...
  const diaryEntries = [
    {
      ...
      workforce_count: 8, weather: 'Clear',
    },
    {
    ...
    },
    {
      ...
      workforce_count: 8, weather: 'Overcast',
    },
    {
      ...
      workforce_count: 6, weather: 'Sunny',
    },
    {
      ...
      workforce_count: 3, weather: 'Windy',
    },
  ]
  ...
  console.log(`App URL: ${SUPABASE_URL.replace('supabase.co', 'vercel.app').replace(/\/.*/, '')}\n`)
}
